...
exports.purchaseEntry = async function (req, res, next) {
  try {
    ...
    let data = sortEdData.map(async (e) => {
      ...
      let update = await caratcounter.findByIdAndUpdate(
        result._id,
        updatedData
      );
...
exports.update_entry = async function (req, res, next) {
  try {
    let newdata;
    if (req.params.type == "bill") {
      newdata = await Pur_entry.findOneAndUpdate(
        { bill_no: req.params.no },
        req.body
      );
    } else if (req.params.type == "inv") {
      newdata = await Pur_entry.findOneAndUpdate(
        { inv_no: req.params.no },
        req.body
      );
    }
    res.status(200).json({
...
exports.delete_entry = async function (req, res, next) {
  try {
    let newdata;
    if (req.params.type == "bill") {
      newdata = await Pur_entry.findOneAndDelete(
        { bill_no: req.params.no },
        req.body
      );
    } else if (req.params.type == "inv") {
...
exports.purchaseReport = async function (req, res) {
  try {
    // console.log(req.body.priceCt[0]);

    // console.log(req.body.priceCt[1]);
    // let body = req.body;
    // let filteredData =  Pur_entry.aggregate([
    let filteredPur = await Pur_entry.aggregate([
    ...
    filteredPur.map((item) => {
      filteredPur_ref_array.push(item.trans_entries.refno);
    });
